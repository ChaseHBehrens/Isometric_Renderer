...
def update():
    ...
    for i in range(len(visible_tiles)):
        for j in range(len(world[tuple(visible_tiles[i])])):
            if (not type(world[tuple(visible_tiles[i])][j]) == tile) and ((visible_tiles[i][0],visible_tiles[i][1],visible_tiles[i][2]-1) in world):
    ...
    for event in pygame.event.get():
        if event.type == KEYDOWN:
            ...
            if event.key == K_SPACE:
                try:
                    del(world[tuple(camera[:3])])
                except:
                    world[tuple(camera[:3])] = [tile('cube')]
                update()
